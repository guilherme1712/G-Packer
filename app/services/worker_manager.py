...
class WorkerManager:
    ...
    @classmethod
    def get_mapping_executor(cls):
        """Pool dedicado para Mapeamento de Cache (Background DB)"""
        if cls._mapping_executor is None:
            cls._mapping_executor = ThreadPoolExecutor(
                max_workers=cls.MAX_MAPPING_WORKERS,
                thread_name_prefix="MapCacheWorker"
            )
        return cls._mapping_executor
    ...
    def shutdown_all(cls):
        ...
        if cls._mapping_executor: cls._mapping_executor.shutdown(wait=False)
