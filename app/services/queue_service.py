    ...
    def _upload_worker(app, task_id):
        ...
        with app.app_context():
            ...
            try:
                ...
                # --- ATUALIZA CACHE LOCAL (Com proteção) ---
                if upload_record.file_id:
                    try:
                        cache_uploaded_item(creds, upload_record.file_id)
                    except Exception as cache_err:
                        print(f"[QueueService] Aviso: Upload OK, mas falha ao atualizar cache: {cache_err}")
                ...
                try:
                    ...
                    print(f"[Thread-{task_id}] Erro DB Update Final: {db_err}")
                    db.session.rollback()
